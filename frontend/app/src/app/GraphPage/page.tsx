...
export default function MainAttraction() {
    ...
    const y = d3.scaleLinear(d3.extent(UseInfoAboutTheGraph.data), [UseInfoAboutTheGraph.height - UseInfoAboutTheGraph.marginBottom, UseInfoAboutTheGraph.marginTop]);
    ...
    return (<>
        ...
        <svg fill="white" className=" fill-white text-orange-400" width={UseInfoAboutTheGraph.width} height={UseInfoAboutTheGraph.height}>
            <path fill="none" stroke="currentColor" strokeWidth="1.5" d={line(UseInfoAboutTheGraph.data)} />
            <g fill="white" stroke="currentColor" strokeWidth="1.5">
                {UseInfoAboutTheGraph.data.map((d: d3.NumberValue, i: string | number | bigint | { valueOf(): number; } | null | undefined) => (<circle key={i} cx={x(i)} cy={y(d)} r="28" />))}
            </g>
        </svg>
        </div>
        </>
